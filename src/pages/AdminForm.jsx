...
import { createLawyers, updateLawyers } from "../helpers/general";
...
export const AdminForm = () => {
  const { userAuth } = CTX();
  const { id } = useParams();
  const lawyerToEdit = id && JSON.parse(Cookies.get("lawyerToEdit"));
  console.log(lawyerToEdit);

  const [isLoading, setIsLoading] = useState(false);
  const navigate = useNavigate();
  const [nonComplexApplicationDatas, setNonComplexApplicationDatas] = useState({
    fullName: lawyerToEdit ? lawyerToEdit.fullName : "",
    practiceArea: lawyerToEdit ? lawyerToEdit.practiceAreas : "",
    location: lawyerToEdit ? lawyerToEdit.location : "",
    professionalMembership: lawyerToEdit
      ? lawyerToEdit.professionalMembership
      : "",
    year: new Date().getFullYear(),
    role: lawyerToEdit ? lawyerToEdit.role : "",
    descriptions: lawyerToEdit ? lawyerToEdit.description : "",
    educationalQuaifications: lawyerToEdit
      ? lawyerToEdit.educationalQualification
      : "",
    email: lawyerToEdit ? lawyerToEdit.contactEmail : "",
    addsOn: lawyerToEdit ? lawyerToEdit.addons : "",
  });
  ...
  const submitData = async (e) => {
    ...
    try {
      setIsLoading(true);
      if (lawyerToEdit) {
        await updateLawyers(totalApplicationData, userAuth, lawyerToEdit._id);
      } else {
        await createLawyers(totalApplicationData, userAuth);
      }
      setIsLoading(false);
      navigate("/admin/lawfirm/kanu_agabi_dashboard/");
    } catch (error) {
      // console.log(error);
    }
  };
  ...
  return (
    <section className="mx-auto w-[96%] max-w-[500px] mt-20">
      <h2 className="text-center text-4xl">Create Lawyer form</h2>
      <form
      ...
      >
        ...
        <div className="w-100%">
          ...
          <input
            ...
            value={nonComplexApplicationDatas.educationalQuaifications}
          />
          <span className="flex my-2">
            <AiOutlineInfoCircle className="w-5 h-5" /> &nbsp; multiple entries
        ...
        <div className="w-100%">
          ...
          <div className="">
            <input
              ...
              name=""
              id=""
              value={nonComplexApplicationDatas.professionalMembership}
              onChange={(e) =>
                setNonComplexApplicationDatas({
                  ...nonComplexApplicationDatas,
                  professionalMembership: e.currentTarget.value,
                })
              }
            />
            <span className="flex my-2">
              <AiOutlineInfoCircle className="w-5 h-5" /> &nbsp; multiple
              entries should be separated with commas
            </span>
          </div>
        </div>
        <div className="w-100%">
